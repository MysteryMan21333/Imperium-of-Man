/obj/item/binoculars
	...
	icon = 'modular_imperium/master_files/icons/obj/items/binoculars.dmi'
	icon_state = "binoculars"
	worn_icon_list = list(
		slot_l_hand_str = 'modular_imperium/master_files/icons/mob/inhands/equipment/binoculars_left.dmi',
		slot_r_hand_str = 'modular_imperium/master_files/icons/mob/inhands/equipment/binoculars_right.dmi',
	)
	atom_flags = CONDUCT
	force = 5
...
/obj/item/binoculars/tactical/proc/acquire_target(atom/A, mob/living/carbon/human/user)
	...
	switch(mode)
		...
		if(MODE_RAILGUN)
			...
			if((GLOB.guardsman_main_ship?.rail_gun?.last_firing + COOLDOWN_RAILGUN_FIRE) > world.time)
				...
				while(laser)
					GLOB.guardsman_main_ship?.rail_gun?.fire_rail_gun(TU,user)
...
/obj/item/binoculars/tactical/proc/try_fire_ob(atom/A, mob/living/carbon/human/user)
	...
	var/warhead_type = GLOB.guardsman_main_ship.orbital_cannon.tray.warhead.name
