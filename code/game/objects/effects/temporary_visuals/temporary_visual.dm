...
///Image that appears at the Tyranid Rally target; only Tyranids can see it
/obj/effect/temp_visual/tyranid/tyranid_tracker_target
	name = "tyranid tracker target"
	icon_state = "nothing"
	duration = TYRANID_HEALTH_ALERT_POINTER_DURATION
	mouse_opacity = MOUSE_OPACITY_TRANSPARENT
	color = COLOR_RED
...
/obj/effect/temp_visual/tyranid/tyranid_tracker_target/Initialize(mapload, atom/target)
	. = ..()
	prepare_huds()
	...
	hud_set_tyranid_tracker_target(target)

/obj/effect/temp_visual/tyranid/tyranid_tracker_target/Destroy()
	if(tracker_target && holder) //Check to avoid runtimes
		tracker_target.overlays -= holder //remove the overlay
	for(var/datum/atom_hud/tyranid_tactical/tyranid_tac_hud in GLOB.huds)
...
/obj/effect/temp_visual/tyranid/tyranid_tracker_target/proc/hud_set_tyranid_tracker_target(atom/target)
	holder = hud_list[TYRANID_TACTICAL_HUD]
	if(!holder)
		return
	...
	hud_list[TYRANID_TACTICAL_HUD] = holder
...
/obj/effect/temp_visual/dropship_flyby
	icon = 'modular_imperium/master_files/icons/obj/structures/prop/dropship.dmi'
...
/obj/effect/temp_visual/dropship_flyby/chaos
