...
/obj/item/paper/tutorial/medical
	...
	At high enough quantities, they may cause vomiting. They do not cause as much pain as burns, but the danger is in what is actually causing them, such as tyranid neurotoxins which can quickly build up in the body after an attack by sentinels, defilers, boilers, etc. In order to treat this, first care must be taken to purge all toxins in the body through dylovene or hypervene.<BR>
	<b>Oxygen</b> = Caused by low blood or certain toxic reagents. The lower the blood, the faster it will build up. Naturally heals on its own. Guardsmans in critical condition will slowly build up oxygen damage until they are dead or brought up to a stable condition.<BR>
	<b>Cloneloss</b> = Caused by tyranids psydraining or cocooning dead guardsmans, or usage of specific chemicals.<BR>
	...
	<b>Alien Embryo</b> = The person has been jumped onto by a hugger time ago. Depending on how long ago, he may be close to bursting. Surgery is the only way to remove the embryo, and a stasis bed will delay the timer considerably. Upon bursting, the player will die and likely receive extremely high amounts of internal bleeding and organ damage, but will stil be revivable should said injuries be healed. Death will pause the timer, so it is not unheard of for guardsmans to dliberatedly kill the victim, put him in a stasis bag, and medevac him for removal while dead before revival, or simply letting him burst before sending his body shipside. Sending the guardsman shipside alive, even moreso if time had passed, is EXTREMELY risky due to the possibility of said guardsman bursting and the larva escaping into the ship, which will evolve into a tyranid and spill acid and destroying critical infrastrcture such as the communications room, potentially leaving the entire guardsman force deaf to each other.<BR>
	...
	Note: Advanced brute and burn kits are for corpsman use due to the additional bonus in healing when applied. This is due to corpsmen having higher medical stats. Normal guardsmans have a delay on usage with no bonus healing, and should stick to gauzes and ointments.<BR>
	...
	<b>Splints</b> = These can be applied to a fractured limb to prevent its symptoms. Splints can be broken should the limb take damage. Splints applied by corpsmen can take more hits than those applied by normal guardsmans. Highly recommended to ask a corpsman to apply it, as self-splinting has great delay.<BR>
	...
	<b>Inaprovaline</b> = Prevents oxygen damage. At >15u, will give a massive heal to guardsmans in critical condition, most of the time enough to bring them up to their feet.<BR>"}

/obj/item/paper/tutorial/mechanics
	name = "Guardsman manual"
	info = {"Up to date as of December 27th '22.<BR>
	<BR>
	...
	<BR>
	Weapon damage has three attributes: Damage, penetration, and sunder. The way damage is calculated is by taking said value and multiplying it by the amount of armor the tyranid has, IE a tyranid with 40 bullet armor will receive only 60% of the advertised damage. Penetration is the amount of armor that will be reduced from the tyranid before the aforementioned calculation. Sunder is the amount of armor that will be "stripped" from the tyranid that it will then have to heal, so consecutive shots will chip away at armor. You can test all guns and spawn tyranids to try them on if you are a ghost/observer, by going to the Ghost tab and clicking "Join Valhalla".<BR>
	<BR>
	...
	<BR>
	<b>Removal of huggers</b>: If a guardsman has been jumped on by a hugger, you will hear a muffled scream as well as the guardsman dropping to the ground. There are two ways to remove a hugger before it implants an embryo: First, click on him with an empty hand on help intent. This will require a second or two of staying still, where a bar will begin to fill. Once complete, a <b>STILL ACTIVE</b> hugger will drop beneath the guardsman, which will need to be killed ASAP before it jumps on someone else. This can be accomplished by simply clicking it on anything that is capable of attacking, such as a knife or your gun. The second method is to set the guardsman on fire; once the hugger has stayed enough to implant an embryo, instead a <b>dead, inactive</b> hugger will drop beneath the guardsman, so long as he is not extinguished. Confirmation as to wether the guardsman has been hugged or not can be accomplished by a quick medical scan.<BR>
	<BR>
	Every guardsman will have their boots paired with a knife inside. This knife can be taken out for any usage, though the most common will be to clear weed sacs or resin walls. They can be attacked, and though tedious, can be cleared if enough guardsmans simply hit them with said knife. Resin walls have hitpoints, they can even be shot down if enough ammunition is spent. Tyranids lurk inside large amounts of resin walls, so make sure they are cleared, and encourage your fellow guardsmans to clear the walls with the boot knife everyone may be carrying. Their gun not having a bayonet is no excuse.<BR>
	<BR>
	The guardsman vendor often gives guardsmans an MRE. This item is deceptively important. A lack of nourishment (yellow or even red bar icon on the right side of the screen under a hamburger) will give a severe mobility impact. This bar goes down over time, and low levels of blood will also deplete it. Carrying one or even two MREs will last you the duration of the operation, and will even help recover your blood partly should you have already fixed what caused said bloodloss. MREs are the most space-efficient food item you can bring with you, and you may even find yourself sharing a piece of it with someone else should they look like a used ketchup packet.<BR>
	<BR>
	Tyranid weeds come in three different types: The basic ones, the green-looking ones, and the web-looking ones. The first are the most common, allowing tyranids to heal if they rest on them, letting them build resin structures, and also giving them a speed bonus while running on them. The bubbly-green looking ones forfeit the speed bonus but allow them to heal much faster while resting on them. The web-looking ones don't give them a speed nor heal bonus, but instead <b>dramatically slow down guardsmans who walk on them</b>. These weeds may also be covered in "sticky resin", which also slows down guardsmans. Worse still, Carriers have the ability to dig holes into the resin and hide huggers in them, but clearing the weeds will automatically destroy said holes and force the hugger out, which will still need to be killed off in any viable manner. There is absolutely zero benefit to leaving weeds intact if you can destroy them, and heavily cluttered territory can hide huggers that will ruin a guardsman push. Since they grow from weed sacs, you only need to destroy the sacs, the rest of the weeds being safely ignorable save for stepping into a hugger hole in heavily cluttered areas.<BR>
	<BR>
	Ammunition is infinite for normal guns. You're heavily encouraged to bring lots and lots of ammunition for your gun of choice. Ammunition can be packed in bulk by taking an ammunition box (Weapon vendor, box tab), dispensing lots of your magazines, and clicking on the pile with the box. This will automatically fill the box with all it can take. Repeat this as you'd like, ammunition never runs out on the vendors, but you may still run out of your pile down in the planet if you didn't pack enough. Take enough to make sure that even if other guardsmans who took the same gun grab it, you won't run out of your own magazines.<BR>
	<BR>
...
/obj/item/paper/tyranidresearch1
	name = "Project X^2.I, 1"
	info = {"This is Tyranidbiologist Dr. Evan Reiker, reporting our current progress in the X^2 project. From the samples we've been able to grow in the lab, we've discovered a lot about these creatures. <BR>
...
/obj/item/paper/tyranidresearch2
...
/obj/item/paper/tyranidresearch3
...
/obj/item/paper/clockresearch1
	...
	Whatever it is, they need to shut up about it! I've told them over and over again that we're focusing on tyranidbiology, and we can't just shift gears (heh) to some magical cult thing nobody's ever heard of! <BR>
