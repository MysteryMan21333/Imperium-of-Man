


//guardsman gloves

/obj/item/clothing/gloves/guardsman
	name = "guardsman combat gloves"
	desc = "Standard issue guardsman tactical gloves. It reads: 'knit by Guardsman Widows Association'."
	icon_state = "gloves_guardsman"
	siemens_coefficient = 0.6
	permeability_coefficient = 0.05
	cold_protection_flags = HANDS
	heat_protection_flags = HANDS
	min_cold_protection_temperature = GLOVES_MIN_COLD_PROTECTION_TEMPERATURE
	max_heat_protection_temperature = GLOVES_MAX_HEAT_PROTECTION_TEMPERATURE
	armor_protection_flags = HANDS
	soft_armor = list(MELEE = 25, BULLET = 15, LASER = 10, ENERGY = 15, BOMB = 15, BIO = 5, FIRE = 15, ACID = 15)

/obj/item/clothing/gloves/guardsman/black
	name = "black guardsman combat gloves"
	desc = "Standard issue guardsman tactical gloves but black! It reads: 'knit by Guardsman Widows Association'."
	icon_state = "gloves_guardsman_black"
	worn_icon_state = "black"

/obj/item/clothing/gloves/guardsman/fingerless
	name = "fingerless guardsman combat gloves"
	desc = "Standard issue guardsman tactical gloves but fingerless! It reads: 'knit by Guardsman Widows Association'."
	icon_state = "gloves_guardsman_fingerless"
	worn_icon_state = "fingerless"

/obj/item/clothing/gloves/guardsman/hyperscale
	name = "8E Chameleon TGMC combat gloves"
	desc = "Standard issue guardsman tactical gloves BUT colorable with a facepaint! It reads: 'knit by Guardsman Widows Association'."
	icon_state = "hyperscale_glove_inhand"
	worn_icon_state = "hyperscale_glove_mob"
	worn_icon_list = list(slot_gloves = /datum/greyscale_config/marine_uniform)
	greyscale_colors = ARMOR_PALETTE_BLACK
	greyscale_config = /datum/greyscale_config/marine_uniform
	colorable_colors = ARMOR_PALETTES_LIST
	colorable_allowed = ICON_STATE_VARIANTS_ALLOWED|PRESET_COLORS_ALLOWED


/obj/item/clothing/gloves/guardsman/hyperscale/color_item(obj/item/facepaint/paint, mob/user)
	. = ..()
	if(!ishuman(user))
		return
	var/mob/living/carbon/human/human = user
	human.regenerate_icons()

/obj/item/clothing/gloves/guardsman/insulated
	name = "insulated guardsman combat gloves"
	desc = "Insulated guardsman tactical gloves that protect against electrical shocks."
	icon_state = "gloves_guardsman_insulated"
	siemens_coefficient = 0

/obj/item/clothing/gloves/guardsman/officer
	name = "officer gloves"
	desc = "Shiny and impressive. They look expensive."
	icon_state = "black"

/obj/item/clothing/gloves/guardsman/officer/chief
	name = "chief officer gloves"
	desc = "Blood crusts are attached to its metal studs, which are slightly dented."

/obj/item/clothing/gloves/guardsman/officer/chief/sa
	name = "spatial agent's gloves"
	desc = "Gloves worn by a Spatial Agent."
	siemens_coefficient = 0
	permeability_coefficient = 0

/obj/item/clothing/gloves/guardsman/techofficer
	name = "tech officer gloves"
	desc = "Sterile AND insulated! Why is not everyone issued with these?"
	icon_state = "yellow"
	siemens_coefficient = 0
	permeability_coefficient = 0.01

/obj/item/clothing/gloves/guardsman/techofficer/captain
	name = "captain's gloves"
	desc = "You may like these gloves, but THEY think you are unworthy of them."
	icon_state = "captain"

/obj/item/clothing/gloves/guardsman/specialist
	name = "\improper B18 defensive gauntlets"
	desc = "A pair of heavily armored gloves."
	icon_state = "armored"
	item_flags = SYNTH_RESTRICTED
	soft_armor = list(MELEE = 35, BULLET = 15, LASER = 15, ENERGY = 15, BOMB = 25, BIO = 15, FIRE = 15, ACID = 20)
	resistance_flags = UNACIDABLE

/obj/item/clothing/gloves/guardsman/specialist/tdf
	icon_state = "armored_tdf"

/obj/item/clothing/gloves/guardsman/veteran/pmc
	name = "armored gloves"
	desc = "Armored gloves used in special operations. They are also insulated against electrical shock."
	icon_state = "black"
	siemens_coefficient = 0
	item_flags = SYNTH_RESTRICTED
	soft_armor = list(MELEE = 30, BULLET = 20, LASER = 20, ENERGY = 20, BOMB = 30, BIO = 20, FIRE = 20, ACID = 15)

/obj/item/clothing/gloves/guardsman/veteran/pmc/commando
	name = "\improper PMC commando gloves"
	desc = "A pair of heavily armored, insulated, acid-resistant gloves."
	icon_state = "death_squad"
	soft_armor = list(MELEE = 40, BULLET = 20, LASER = 20, ENERGY = 20, BOMB = 30, BIO = 20, FIRE = 20, ACID = 25)
	resistance_flags = UNACIDABLE

/obj/item/clothing/gloves/guardsman/chaos
	name = "\improper CHAOS gloves"
	desc = "Gloves with origins dating back to the old mining colonies, they look pretty tough."
	icon_state = "chaos"

/obj/item/clothing/gloves/guardsman/chaos/insulated
	name = "\improper Insulated CHAOS gloves"
	desc = "Gloves with origins dating back to the old mining colonies. These ones appear to have an electrically insulating layer built into them."
	siemens_coefficient = 0

/obj/item/clothing/gloves/guardsman/chaos/veteran
	name = "\improper CHAOS veteran gloves"
	desc = "Gloves with origins dating back to the old mining colonies. These ones seem tougher than normal."
	icon_state = "chaos_veteran"
	soft_armor = list(MELEE = 30, BULLET = 20, LASER = 15, ENERGY = 20, BOMB = 15, BIO = 5, FIRE = 15, ACID = 15)

/obj/item/clothing/gloves/guardsman/chaos/officer
	name = "\improper CHAOS gloves"
	desc = "Black gloves commonly worn by CHAOS officers."
	icon_state = "chaos_officer_gloves"

/obj/item/clothing/gloves/guardsman/icc
	name = "\improper ICC gloves"
	desc = "Tough looking working gloves."
	icon_state = "icc"

/obj/item/clothing/gloves/guardsman/icc/insulated
	name = "\improper ICC insulated gloves"
	desc = "Tough looking working gloves. These ones appear to have insulation to protect from electric shock."
	siemens_coefficient = 0

/obj/item/clothing/gloves/guardsman/icc/guard
	name = "\improper ICCGF gloves"
	desc = "Tough looking tactical gloves."
	icon_state = "icc_guard"
	soft_armor = list(MELEE = 30, BULLET = 20, LASER = 15, ENERGY = 20, BOMB = 15, BIO = 5, FIRE = 15, ACID = 15)

/obj/item/clothing/gloves/guardsman/commissar
	name = "\improper commissar gloves"
	desc = "Gloves worn by commissars of the Imperial Army so that they do not soil their hands with the blood of their men."
	icon_state = "red"
	soft_armor = list(MELEE = 35, BULLET = 30, LASER = 30, ENERGY = 30, BOMB = 15, BIO = 10, FIRE = 20, ACID = 20)

/obj/item/clothing/gloves/guardsman/tdf
	icon_state = "tdf"
