...
/datum/action/ability/activable/tyranid/pounce/runner
	...
	keybinding_signals = list(
		KEYBINDING_NORMAL = COMSIG_TYRANIDABILITY_RUNNER_POUNCE,
		KEYBINDING_ALTERNATE = COMSIG_TYRANIDABILITY_TOGGLE_SAVAGE,
	)
...
/datum/action/ability/activable/tyranid/pounce/runner/give_action(mob/living/L)
	. = ..()
...
/datum/action/ability/activable/tyranid/pounce/runner/alternate_action_activate()
...
/datum/action/ability/activable/tyranid/pounce/runner/trigger_pounce_effect(mob/living/living_target)
	. = ..()
	if(!savage_activated)
		return
	...
	var/savage_damage = max(RUNNER_SAVAGE_DAMAGE_MINIMUM, tyranid_owner.plasma_stored * 0.15)
	var/savage_cost = savage_damage * 2
	if(tyranid_owner.plasma_stored < savage_cost)
		owner.balloon_alert(owner, "Not enough plasma to Savage ([savage_cost])")
		return
	living_target.attack_alien_harm(tyranid_owner, savage_damage)
	tyranid_owner.use_plasma(savage_cost)
...
/datum/action/ability/activable/tyranid/pounce/runner/process()
	if(COOLDOWN_CHECK(src, savage_cooldown))
...
/datum/action/ability/tyranid_action/evasion
	...
	keybinding_signals = list(
		KEYBINDING_NORMAL = COMSIG_TYRANIDABILITY_EVASION,
		KEYBINDING_ALTERNATE = COMSIG_TYRANIDABILITY_AUTO_EVASION,
	)
...
/datum/action/ability/tyranid_action/evasion/on_cooldown_finish()
	. = ..()
	owner.balloon_alert(owner, "Evasion ready")
	owner.playsound_local(owner, 'sound/effects/alien/new_larva.ogg', 25, 0, 1)

/datum/action/ability/tyranid_action/evasion/can_use_action(silent = FALSE, override_flags)
	. = ..()
	var/mob/living/carbon/tyranid/tyranid_owner = owner
	if(tyranid_owner.on_fire)
		if(!silent)
			tyranid_owner.balloon_alert(tyranid_owner, "Can't while on fire!")
		return FALSE

/datum/action/ability/tyranid_action/evasion/alternate_action_activate()
...
/datum/action/ability/tyranid_action/evasion/action_activate()
	...
	RegisterSignal(owner, COMSIG_TYRANID_PROJECTILE_HIT, PROC_REF(evasion_dodge))
...
/datum/action/ability/tyranid_action/evasion/ai_should_start_consider()
	return TRUE

/datum/action/ability/tyranid_action/evasion/ai_should_use(atom/target)
	if(iscarbon(target))
		return FALSE
	var/mob/living/carbon/tyranid/tyranid_owner = owner
	var/hp_left_percent = tyranid_owner.health / tyranid_owner.maxHealth // minimum_health or retreating ai datum instead maybe?
	return (hp_left_percent < 0.5)

/datum/action/ability/tyranid_action/evasion/process()
	var/mob/living/carbon/tyranid/runner/runner_owner = owner
...
*/
/datum/action/ability/tyranid_action/evasion/proc/evasion_flamer_hit(datum/source, obj/projectile/proj)
	SIGNAL_HANDLER
...
*/
/datum/action/ability/tyranid_action/evasion/proc/evasion_debuff_check(datum/source, amount)
	SIGNAL_HANDLER
...
/datum/action/ability/tyranid_action/evasion/proc/evasion_deactivate()
...
/datum/action/ability/tyranid_action/evasion/proc/evasion_throw_dodge(datum/source, atom/movable/proj)
	SIGNAL_HANDLER
	var/mob/living/carbon/tyranid/tyranid_owner = owner
	if(!evade_active) //If evasion is not active we don't dodge
		return NONE
	if((tyranid_owner.last_move_time < (world.time - RUNNER_EVASION_RUN_DELAY))) //Gotta keep moving to benefit from evasion!
		return NONE
	if(isitem(proj))
		var/obj/item/I = proj
...
/datum/action/ability/tyranid_action/evasion/proc/evasion_dodge(datum/source, obj/projectile/proj, cardinal_move, uncrossing)
	SIGNAL_HANDLER
	if(!evade_active) //If evasion is not active we don't dodge
		return FALSE
	var/mob/living/carbon/tyranid/tyranid_owner = owner
	if((tyranid_owner.last_move_time < (world.time - RUNNER_EVASION_RUN_DELAY))) //Gotta keep moving to benefit from evasion!
		return FALSE
	if(tyranid_owner.issametyranidhive(proj.firer)) //We automatically dodge allied projectiles at no cost, and no benefit to our evasion stacks
		return COMPONENT_PROJECTILE_DODGE
	if(proj.ammo.ammo_behavior_flags & AMMO_FLAME) //We can't dodge literal fire
		return FALSE
	if(proj.original_target == tyranid_owner && proj.distance_travelled < 2) //Pointblank shot.
		return FALSE
	if(!tyranid_owner.fire_stacks)
...
/datum/action/ability/tyranid_action/evasion/proc/evasion_dodge_fx(atom/movable/proj)
	var/mob/living/carbon/tyranid/tyranid_owner = owner
	tyranid_owner.visible_message(span_warning("[tyranid_owner] effortlessly dodges the [proj.name]!"), \
	span_tyraniddanger("We effortlessly dodge the [proj.name]![(RUNNER_EVASION_COOLDOWN_REFRESH_THRESHOLD - evasion_stacks) > 0 && evasion_stacks > 0 ? " We must dodge [RUNNER_EVASION_COOLDOWN_REFRESH_THRESHOLD - evasion_stacks] more projectile damage before [src]'s cooldown refreshes." : ""]"))
	tyranid_owner.add_filter("runner_evasion", 2, gauss_blur_filter(5))
	addtimer(CALLBACK(tyranid_owner, TYPE_PROC_REF(/datum, remove_filter), "runner_evasion"), 0.5 SECONDS)
	tyranid_owner.do_jitter_animation(4000)
	if(evasion_stacks >= RUNNER_EVASION_COOLDOWN_REFRESH_THRESHOLD && cooldown_remaining()) //We have more evasion stacks than needed to refresh our cooldown, while being on cooldown.
		clear_cooldown()
		if(auto_evasion && tyranid_owner.plasma_stored >= ability_cost)
			action_activate()
	var/turf/current_turf = get_turf(tyranid_owner) //location of after image SFX
	...
	for(var/i=0 to 2) //number of after images
		...
		after_image.pixel_x = pick(randfloat(tyranid_owner.pixel_x * 3, tyranid_owner.pixel_x * 1.5), rand(0, tyranid_owner.pixel_x * -1)) //Variation on the X position
...
/datum/action/ability/activable/tyranid/snatch
	...
	keybinding_signals = list(
		KEYBINDING_NORMAL = COMSIG_TYRANIDABILITY_SNATCH,
	)
...
/datum/action/ability/activable/tyranid/snatch/action_activate()
	if(!stolen_item)
		return ..()
	drop_item()

/datum/action/ability/activable/tyranid/snatch/can_use_ability(atom/A, silent, override_flags)
	. = ..()
	if(!.)
		return
	if(!owner.Adjacent(A))
...
/datum/action/ability/activable/tyranid/snatch/use_ability(atom/A)
...
/datum/action/ability/activable/tyranid/snatch/proc/owner_turned(datum/source, old_dir, new_dir)
	SIGNAL_HANDLER
...
///Force the tyranid owner to drop the stolen item
/datum/action/ability/activable/tyranid/snatch/proc/drop_item()
	if(!stolen_item)
		return
