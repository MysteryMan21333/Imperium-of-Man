...
/datum/action/ability/tyranid_action/baneling_explode
	...
	action_icon = 'modular_imperium/master_files/icons/tyranid/actions/baneling.dmi'
	desc = "Explode and spread dangerous toxins to hinder or kill your foes. You die."
	keybinding_signals = list(
		KEYBINDING_NORMAL = COMSIG_TYRANIDABILITY_BANELING_EXPLODE,
	)
	/// List of available reagants for baneling
	var/static/list/baneling_smoke_list = list(
		/datum/reagent/toxin/tyranid_neurotoxin = /datum/effect_system/smoke_spread/tyranid/neuro/medium,
		/datum/reagent/toxin/acid = /datum/effect_system/smoke_spread/tyranid/acid,
	)

/datum/action/ability/tyranid_action/baneling_explode/give_action(mob/living/L)
	. = ..()
	var/mob/living/carbon/tyranid/X = L
	RegisterSignal(X, COMSIG_MOB_PRE_DEATH, PROC_REF(handle_smoke))

/datum/action/ability/tyranid_action/baneling_explode/remove_action(mob/living/L)
	. = ..()
	var/mob/living/carbon/tyranid/X = L
	UnregisterSignal(X, COMSIG_MOB_PRE_DEATH)

/datum/action/ability/tyranid_action/baneling_explode/action_activate()
	. = ..()
...
/datum/action/ability/tyranid_action/baneling_explode/proc/handle_smoke(datum/source, ability = FALSE)
	SIGNAL_HANDLER
	var/mob/living/carbon/tyranid/X = owner
...
/datum/action/ability/tyranid_action/baneling_explode/ai_should_start_consider()
	return TRUE

/datum/action/ability/tyranid_action/baneling_explode/ai_should_use(atom/target)
	if(!iscarbon(target))
		return FALSE
...
/datum/action/ability/tyranid_action/select_reagent/baneling
	...
	ability_cost = 0
	keybinding_signals = list(
		KEYBINDING_NORMAL = COMSIG_TYRANIDABILITY_BANELING_CHOOSE_REAGENT,
		KEYBINDING_ALTERNATE = COMSIG_TYRANIDABILITY_RADIAL_SELECT_REAGENT,
	)

/datum/action/ability/tyranid_action/select_reagent/baneling/give_action(mob/living/L)
	. = ..()
	var/mob/living/carbon/tyranid/caster = L
	caster.selected_reagent = GLOB.baneling_chem_type_list[1]
	update_button_icon() //Update immediately to get our default

/datum/action/ability/tyranid_action/select_reagent/baneling/action_activate()
	INVOKE_ASYNC(src, PROC_REF(select_reagent_radial))
	return COMSIG_KB_ACTIVATED

/datum/action/ability/tyranid_action/select_reagent/baneling/select_reagent_radial()
	...
	var/static/list/reagent_images_list = list(
		DEFILER_NEUROTOXIN = image('modular_imperium/master_files/icons/tyranid/actions/baneling.dmi', icon_state = DEFILER_NEUROTOXIN),
		BANELING_ACID = image('modular_imperium/master_files/icons/tyranid/actions/baneling.dmi', icon_state = BANELING_ACID),
		)
