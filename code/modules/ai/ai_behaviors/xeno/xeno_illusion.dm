/datum/ai_behavior/tyranid/illusion
...
/datum/ai_behavior/tyranid/illusion/New(loc, parent_to_assign, escorted_atom)
	if(!escorted_atom)
		base_action = MOVING_TO_NODE
	..()

/// We want a separate look_for_new_state in order to make illusions behave as we wish
/datum/ai_behavior/tyranid/illusion/look_for_new_state()
...
/datum/ai_behavior/tyranid/illusion/attack_target(datum/source, atom/attacked)
	if(!attacked)
		attacked = atom_to_walk_to
	var/mob/illusion/illusion_parent = mob_parent
	var/mob/living/carbon/tyranid/original_tyranid = illusion_parent.original_mob
	mob_parent.changeNext_move(original_tyranid.tyranid_caste.attack_delay)
	if(ismob(attacked))
