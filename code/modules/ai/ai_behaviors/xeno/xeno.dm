//Generic template for application to a tyranid/ mob, contains specific obstacle dealing alongside targeting only humans, tyranids of a different hive and sentry turrets

/datum/ai_behavior/tyranid
	sidestep_prob = 25
...
/datum/ai_behavior/tyranid/New(loc, parent_to_assign, escorted_atom, can_heal = TRUE)
	..()
	refresh_abilities()
	mob_parent.a_intent = INTENT_HARM //Killing time
	src.can_heal = can_heal

/datum/ai_behavior/tyranid/start_ai()
	...
	RegisterSignal(mob_parent, COMSIG_TYRANID_TAKING_DAMAGE, PROC_REF(check_for_critical_health))
	return ..()

///Refresh abilities-to-consider list
/datum/ai_behavior/tyranid/proc/refresh_abilities()
	SIGNAL_HANDLER
	ability_list = list()
...
/datum/ai_behavior/tyranid/process()
	...
	for(var/datum/action/action in ability_list)
		if(!action.ai_should_use(atom_to_walk_to))
			continue
		//tyranid_action/activable is activated with a different proc for keybinded actions, so we gotta use the correct proc
		if(istype(action, /datum/action/ability/activable/tyranid))
			var/datum/action/ability/activable/tyranid/tyranid_action = action
			tyranid_action.use_ability(atom_to_walk_to)
		else
...
/datum/ai_behavior/tyranid/look_for_new_state()
	...
	switch(current_action)
		if(ESCORTING_ATOM)
			if(get_dist(escorted_atom, mob_parent) > ESCORTING_MAX_DISTANCE)
				look_for_next_node()
				return
			var/atom/next_target = get_nearest_target(escorted_atom, target_distance, TARGET_HOSTILE, mob_parent.faction, mob_parent.get_tyranid_hivenumber())
			if(!next_target)
				return
			change_action(MOVING_TO_ATOM, next_target)
		if(MOVING_TO_NODE, FOLLOWING_PATH)
			var/atom/next_target = get_nearest_target(mob_parent, target_distance, TARGET_HOSTILE, mob_parent.faction, mob_parent.get_tyranid_hivenumber())
			if(!next_target)
				...
				if(!goal_node) // We are randomly moving
					var/atom/tyranid_to_follow = get_nearest_target(mob_parent, ESCORTING_MAX_DISTANCE, TARGET_FRIENDLY_TYRANID, mob_parent.faction, mob_parent.get_tyranid_hivenumber())
					if(tyranid_to_follow)
						set_escorted_atom(null, tyranid_to_follow, TRUE)
						return
				return
			change_action(MOVING_TO_ATOM, next_target)
		if(MOVING_TO_ATOM)
...
/datum/ai_behavior/tyranid/deal_with_obstacle(datum/source, direction)
	...
	for(var/obj/structure/obstacle in obstacle_turf.contents)
		if(obstacle.dir & direction && obstacle.resistance_flags & TYRANID_DAMAGEABLE)
			INVOKE_ASYNC(src, PROC_REF(attack_target), null, obstacle)
			return COMSIG_OBSTACLE_DEALT_WITH

/datum/ai_behavior/tyranid/cleanup_current_action(next_action)
	. = ..()
	if(next_action == MOVING_TO_NODE)
		return
	if(!istyranid(mob_parent))
		return
	var/mob/living/living_mob = mob_parent
	if(can_heal && living_mob.resting)
		SEND_SIGNAL(mob_parent, COMSIG_TYRANIDABILITY_REST)
		UnregisterSignal(mob_parent, COMSIG_TYRANID_HEALTH_REGEN)

/datum/ai_behavior/tyranid/cleanup_signals()
	. = ..()
	UnregisterSignal(mob_parent, COMSIG_OBSTRUCTED_MOVE)
	UnregisterSignal(mob_parent, list(ACTION_GIVEN, ACTION_REMOVED))
	UnregisterSignal(mob_parent, COMSIG_TYRANID_TAKING_DAMAGE)

///Signal handler to try to attack our target
/datum/ai_behavior/tyranid/proc/attack_target(datum/source, atom/attacked)
	SIGNAL_HANDLER
...
/datum/ai_behavior/tyranid/register_action_signals(action_type)
	switch(action_type)
		if(MOVING_TO_ATOM)
...
/datum/ai_behavior/tyranid/unregister_action_signals(action_type)
	switch(action_type)
		if(MOVING_TO_ATOM)
...
/datum/ai_behavior/tyranid/proc/try_to_heal()
	var/mob/living/carbon/tyranid/living_mob = mob_parent
	if(!living_mob.loc_weeds_type)
		if(living_mob.resting)//We are resting on no weeds
			SEND_SIGNAL(mob_parent, COMSIG_TYRANIDABILITY_REST)
			UnregisterSignal(mob_parent, list(COMSIG_TYRANID_HEALTH_REGEN, COMSIG_TYRANID_PLASMA_REGEN))
		return FALSE
	if(living_mob.resting)//Already resting
		...
		return TRUE
	SEND_SIGNAL(mob_parent, COMSIG_TYRANIDABILITY_REST)
	RegisterSignal(mob_parent, COMSIG_TYRANID_HEALTH_REGEN, PROC_REF(check_for_health))
	RegisterSignal(mob_parent, COMSIG_TYRANID_PLASMA_REGEN, PROC_REF(check_for_plasma))
	return TRUE

///Wait for the tyranid to be full life and plasma to unrest
/datum/ai_behavior/tyranid/proc/check_for_health(mob/living/carbon/tyranid/healing, list/heal_data)
	SIGNAL_HANDLER
	if(healing.health + heal_data[1] >= healing.maxHealth && healing.plasma_stored >= healing.tyranid_caste.plasma_max * healing.tyranid_caste.plasma_regen_limit)
		SEND_SIGNAL(mob_parent, COMSIG_TYRANIDABILITY_REST)
		UnregisterSignal(mob_parent, list(COMSIG_TYRANID_HEALTH_REGEN, COMSIG_TYRANID_PLASMA_REGEN))

///Wait for the tyranid to be full life and plasma to unrest
/datum/ai_behavior/tyranid/proc/check_for_plasma(mob/living/carbon/tyranid/healing, list/plasma_data)
	SIGNAL_HANDLER
	if(healing.health >= healing.maxHealth && healing.plasma_stored + plasma_data[1] >= healing.tyranid_caste.plasma_max * healing.tyranid_caste.plasma_regen_limit)
		SEND_SIGNAL(mob_parent, COMSIG_TYRANIDABILITY_REST)
		UnregisterSignal(mob_parent, list(COMSIG_TYRANID_HEALTH_REGEN, COMSIG_TYRANID_PLASMA_REGEN))

///Called each time the ai takes damage; if we are below a certain health threshold, try to retreat
/datum/ai_behavior/tyranid/proc/check_for_critical_health(datum/source, damage)
	SIGNAL_HANDLER
	var/mob/living/living_mob = mob_parent
...
/datum/ai_behavior/tyranid/proc/climb_window_frame(turf/window_turf)
	mob_parent.loc = window_turf
	mob_parent.last_move_time = world.time
	LAZYDECREMENT(mob_parent.do_actions, window_turf)

/datum/ai_behavior/tyranid/ranged
	distance_to_maintain = 5
	minimum_health = 0.3

/datum/ai_behavior/tyranid/suicidal
	minimum_health = 0
