	...
	if(href_list["editid"])
		...
		switch(href_list["radioban"])
			if("server")
				roles_to_ban += "Server"
			if("role")
				href_list.Remove("Command", "Police", "Engineering", "Medical", "Guardsmans", "Requisitions", "Silicon", "Abstract", "Antagonist Positions") //remove the role banner hidden input values
