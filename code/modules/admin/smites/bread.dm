...
/datum/smite/bread/effect(client/user, mob/living/target)
	...
	if(tgui_alert(usr, "Irreversibly transform [target] into a delectable loaf of bread?", "Continue?", list("Yes", "No")) != "Yes")
		return
	var/mutable_appearance/bread_appearance = mutable_appearance('modular_imperium/master_files/icons/obj/items/food/bread.dmi', "breadtg")
...
/proc/breadify(atom/movable/target)
	var/obj/item/reagent_containers/food/snacks/sandwiches/guardsmanbread/bread = new(get_turf(target))
