/datum/ammo/tyranid/fireball
	name = "fireball"
	icon_state = "tyranid_fireball"
	damage = 50
	max_range = 5
	ammo_behavior_flags = AMMO_TYRANID|AMMO_SKIPS_ALIENS|AMMO_TARGET_TURF
	bullet_color = null

/datum/ammo/tyranid/fireball/on_hit_mob(mob/target_mob, obj/projectile/proj)
	drop_flame(target_mob)

/datum/ammo/tyranid/fireball/on_hit_obj(obj/target_obj, obj/projectile/proj)
	. = ..()
	drop_flame(target_obj)

/datum/ammo/tyranid/fireball/on_hit_turf(turf/target_turf, obj/projectile/proj)
	. = ..()
	drop_flame(target_turf.density ? proj : target_turf)

/datum/ammo/tyranid/fireball/do_at_max_range(turf/target_turf, obj/projectile/proj)
	. = ..()
	drop_flame(target_turf.density ? proj : target_turf)

/datum/ammo/tyranid/fireball/drop_flame(atom/target_atom)
	new /obj/effect/temp_visual/tyranid_fireball_explosion(get_turf(target_atom))
