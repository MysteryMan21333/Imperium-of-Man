...
/proc/filled_turfs(atom/center, radius = 3, type = "circle", include_edge = TRUE, bypass_window = FALSE, projectile = FALSE, bypass_tyranid = FALSE, air_pass = FALSE)
	var/turf/center_turf = get_turf(center)
	if(radius < 0 || !center)
		return
	if(radius == 0)
		...
		for(var/X in turfs_to_check)
			...
			for(var/direction in directions)
				...
				if (AdjT in results) // Ignore existing turfs
					continue
				if(AdjT.density || LinkBlocked(T, AdjT, bypass_window, projectile, bypass_tyranid, air_pass))
					if(include_edge)
						results += AdjT
					continue
