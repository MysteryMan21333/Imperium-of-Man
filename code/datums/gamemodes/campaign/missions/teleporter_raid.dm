//disabling CHAOS's ability to teleport deploy
/datum/campaign_mission/destroy_mission/teleporter_raid
	...
	objective_destruction_messages = list(
		"last" = list(
			MISSION_STARTING_FACTION = "Bluespace core destroyed, outstanding work guardsmans!",
			MISSION_HOSTILE_FACTION = "Bluespace core destroyed, mission failed. All forces retreat!",
		),
	)
	starting_faction_objective_description = "Major Victory: Destroy the CHAOS Bluespace core at all costs"
	...
	starting_faction_mission_brief = "Intelligence has pinpointed the CHAOS's Bluespace core on this remote lunar base. The core powers all CHAOS teleporter arrays in the system. \
		If we can destroy the core, we'll completely disable the CHAOS's ability to deploy forces into the field, crippling their mobility. \
	...
	starting_faction_additional_rewards = "Permanently disable the CHAOS's ability to deploy via teleportation and impair their logistic network"
...
/datum/campaign_mission/destroy_mission/teleporter_raid/apply_major_victory()
	...
	chaos_team.add_asset(/datum/campaign_asset/teleporter_disabled)
	chaos_team.add_asset(/datum/campaign_asset/attrition_modifier/malus_teleporter)

/datum/campaign_mission/destroy_mission/teleporter_raid/apply_major_loss()
	. = ..()
	var/datum/faction_stats/chaos_team = mode.stat_list[hostile_faction]
	chaos_team.add_asset(/datum/campaign_asset/teleporter_enabled)
	chaos_team.add_asset(/datum/campaign_asset/teleporter_charges)
