GLOBAL_DATUM_INIT(guardsman_main_ship, /datum/guardsman_main_ship, new)

// datum for stuff specifically related to the guardsman main ship like theseus
/datum/guardsman_main_ship
...
/datum/guardsman_main_ship/proc/make_maint_all_access()
...
/datum/guardsman_main_ship/proc/revoke_maint_all_access()
...
/datum/guardsman_main_ship/proc/set_security_level(level, announce = TRUE)
	switch(level)
		if("green")
	...
	if(level <= SEC_LEVEL_BLUE)
		...
		for(var/obj/machinery/light/mainship/light AS in GLOB.mainship_lights)
			...
			if(!A.power_light || light.status != LIGHT_OK) //do not adjust unpowered or broken bulbs
				continue
			light.light_color = COLOR_CHAOSEWHAT_LIGHTER_RED
...
/datum/guardsman_main_ship/proc/get_security_level(sec = security_level)
	switch(sec)
		if(SEC_LEVEL_GREEN)
			return "green"
		if(SEC_LEVEL_BLUE)
...
/datum/guardsman_main_ship/proc/seclevel2num(seclevel)
