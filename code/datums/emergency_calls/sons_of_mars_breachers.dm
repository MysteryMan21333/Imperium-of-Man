//Sons of Mars
/datum/emergency_call/chaos_breachers
...
/datum/emergency_call/chaos_breachers/print_backstory(mob/living/carbon/human/H)
	to_chat(H, "<B>You [pick("grew up in the mines working in horrible conditions until signing up to", "were part of a poor family until you decided to leave to join", "were born on a colony on Mars to a family of several brothers and sisters before leaving to", "worked at horrible conditions in the mines until deciding to leave to join", "have a proud family tradition of military service so enrolled as early as possible to serve")] the Sons of Mars (Chaos).</b>")
	to_chat(H, "<B>Due to your [pick("marked distinction in combat", "ruthless record fighting against TGMC", "family's connections", "previous failures in combat")] you have been assigned to the 5th Special Assault Force of the CHAOS.</b>")
	...
	to_chat(H, "")
	to_chat(H, "<B>Today, a Imperium vessel, [SSmapping.configs[SHIP_MAP].map_name], has sent out a distress signal on the orbit of [SSmapping.configs[GROUND_MAP].map_name]. This is our chance to attack without being intercepted!</b>")
	to_chat(H, "<B>Eliminate the Imperium personnel onboard, capture the ship. If there are fellow ICC contingents such as the ICCAF, then work with them in this goal. Take no prisoners. Take back what was once lost.</B>")

/datum/emergency_call/chaos_breachers/do_activate(announce = TRUE)
	max_specialists = floor(mob_max * 0.2)
	return ..()

/datum/emergency_call/chaos_breachers/create_member(datum/mind/M)
	. = ..()
	if(!.)
		return
	...
	if(!leader)
		leader = H
		selected_job = SSjob.GetJobType(/datum/job/chaos/ert/leader/breacher)
	...
	if(medics < max_medics)
		selected_job = SSjob.GetJobType(/datum/job/chaos/ert/medic/breacher)
		H.apply_assigned_role_to_spawn(selected_job)
	...
	if(max_specialists > 0)
		selected_job = SSjob.GetJobType(/datum/job/chaos/ert/breacher/specialist)
		H.apply_assigned_role_to_spawn(selected_job)
	...
	selected_job = SSjob.GetJobType(/datum/job/chaos/ert/breacher)
	H.apply_assigned_role_to_spawn(selected_job)
