...
/datum/emergency_call/chaos/print_backstory(mob/living/carbon/human/H)
	to_chat(H, "<B>You [pick("grew up in the mines working in horrible conditions until signing up to", "were part of a poor family until you decided to leave to join", "were born on a colony on Mars to a family of several brothers and sisters before leaving to", "worked at horrible conditions in the mines until deciding to leave to join")] the Sons of Mars (Chaos).</b>")
	to_chat(H, "<B>As a member state of the Independent Colonial Confederation (ICC), the Chaos, which formed in 2180, is a heavily industrialized group with a standing army of approx. 200,000 enlisted or trained members.</b>")
...
/datum/emergency_call/chaos/do_activate(announce = TRUE)
	max_specialists = round(mob_max * 0.2)
	return ..()

/datum/emergency_call/chaos/create_member(datum/mind/M)
	. = ..()
	if(!.)
		return
	...
	if(!leader)
		leader = H
		var/datum/job/J = SSjob.GetJobType(/datum/job/chaos/ert/leader)
	...
	if(medics < max_medics)
		var/datum/job/J = SSjob.GetJobType(/datum/job/chaos/ert/medic)
		H.apply_assigned_role_to_spawn(J)
	...
	if(max_specialists > 0)
		var/datum/job/J = SSjob.GetJobType(/datum/job/chaos/ert/specialist)
		H.apply_assigned_role_to_spawn(J)
		...
		return
	if(prob(65))
		var/datum/job/J = SSjob.GetJobType(/datum/job/chaos/ert/veteran)
		H.apply_assigned_role_to_spawn(J)
	...
	var/datum/job/J = SSjob.GetJobType(/datum/job/chaos/ert/standard)
	H.apply_assigned_role_to_spawn(J)
