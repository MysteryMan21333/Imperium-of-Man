...
/datum/outfit/job/chaos/pre_equip(mob/living/carbon/human/H, visualsOnly = FALSE)
	. = ..()
	H.underwear = 10
	H.undershirt = H.undershirt ? 10 : 0
	H.regenerate_icons()

//Base job for normal gameplay CHAOS, not ERT.
/datum/job/chaos/squad
	access = ALL_ANTAGONIST_ACCESS
	minimal_access = ALL_ANTAGONIST_ACCESS
...
/datum/job/chaos/squad/radio_help_message(mob/M)
	. = ..()
	if(istype(SSticker.mode, /datum/game_mode/hvh/combat_patrol))
		if(issensorcapturegamemode(SSticker.mode))
			to_chat(M, span_highdanger("Your platoon has orders to defend sensor towers in the AO and prevent them from reactivation by Imperium forces until heavy reeinforcement arrives. High Command considers the successful prevention of the reactivation of the sensor towers a major victory"))
		else
			to_chat(M, span_highdanger("Your platoon has orders to patrol a remote territory illegally claimed by Imperium imperialists. Intel suggests TGMC units are similarly trying to press their claims by force. Work with your team and eliminate all TGMC you encounter while preserving your own strength! High Command considers wiping out all enemies a major victory, or inflicting more casualties a minor victory."))
		return

/datum/job/chaos/squad/after_spawn(mob/living/carbon/C, mob/M, latejoin = FALSE)
	. = ..()
...
/datum/job/chaos/squad/equip_spawning_squad(mob/living/carbon/human/new_character, datum/squad/assigned_squad, client/player)
	if(!assigned_squad)
...
//CHAOS Standard
/datum/job/chaos/squad/standard
	title = CHAOS_SQUAD_GUARDSMAN
	paygrade = "CHAOS_E1"
	comm_title = "Mar"
	...
	outfit = /datum/outfit/job/chaos/squad/standard
	jobworth = list(
...
/datum/job/chaos/squad/standard/radio_help_message(mob/M)
	. = ..()
	to_chat(M, {"\nYou are a rank-and-file soldier of the Sons of Mars, and that is your strength.
What you lack alone, you gain standing shoulder to shoulder with the men and women of the CHAOS. For Mars!"})

/datum/outfit/job/chaos/squad/standard
	name = "CHAOS Standard"
	jobtype = /datum/job/chaos/squad/standard

	id = /obj/item/card/id/dogtag/chaos


/datum/job/chaos/squad/engineer
	title = CHAOS_SQUAD_ENGINEER
	paygrade = "CHAOS_E3"
	comm_title = "Eng"
	total_positions = 12
	skills_type = /datum/skills/combat_engineer
	display_order = JOB_DISPLAY_ORDER_SUQAD_ENGINEER
	outfit = /datum/outfit/job/chaos/squad/engineer
	job_flags = JOB_FLAG_LATEJOINABLE|JOB_FLAG_ROUNDSTARTJOINABLE|JOB_FLAG_ALLOWS_PREFS_GEAR|JOB_FLAG_PROVIDES_BANK_ACCOUNT|JOB_FLAG_ADDTOMANIFEST|JOB_FLAG_PROVIDES_SQUAD_HUD|JOB_FLAG_CAN_SEE_ORDERS
	jobworth = list(
		/datum/job/tyranid = LARVA_POINTS_REGULAR,
		/datum/job/chaos/squad/veteran = VETERAN_POINTS_REGULAR,
...
/datum/job/chaos/squad/engineer/after_spawn(mob/living/carbon/new_mob, mob/user, latejoin = FALSE)
	. = ..()
	if(!ishuman(new_mob))
		return
	var/mob/living/carbon/human/new_human = new_mob
	...
	switch(playtime_mins)
		...
		if(30001 to INFINITY) // 500 hrs
			new_human.wear_id.paygrade = "CHAOS_S2"

/datum/job/chaos/squad/engineer/radio_help_message(mob/M)
	. = ..()
...
/datum/outfit/job/chaos/squad/engineer
	name = "CHAOS Engineer"
	jobtype = /datum/job/chaos/squad/engineer

	id = /obj/item/card/id/dogtag/chaos


/datum/job/chaos/squad/medic
	title = CHAOS_SQUAD_CORPSMAN
	paygrade = "CHAOS_E3"
	comm_title = "Med"
	total_positions = 16
	minimap_icon = "medic"
	...
	outfit = /datum/outfit/job/chaos/squad/medic
	job_flags = JOB_FLAG_LATEJOINABLE|JOB_FLAG_ROUNDSTARTJOINABLE|JOB_FLAG_ALLOWS_PREFS_GEAR|JOB_FLAG_PROVIDES_BANK_ACCOUNT|JOB_FLAG_ADDTOMANIFEST|JOB_FLAG_PROVIDES_SQUAD_HUD|JOB_FLAG_CAN_SEE_ORDERS
	jobworth = list(
		/datum/job/tyranid = LARVA_POINTS_REGULAR,
		/datum/job/chaos/squad/veteran = VETERAN_POINTS_REGULAR,
	...
	html_description = {"
		...
		<br /><br />
		<b>Duty</b>: Tend the injuries of your fellow guardsmans or related personnel, keep them at fighting strength.
	"}

/datum/job/chaos/squad/medic/after_spawn(mob/living/carbon/new_mob, mob/user, latejoin = FALSE)
	. = ..()
	if(!ishuman(new_mob))
		return
	var/mob/living/carbon/human/new_human = new_mob
...
/datum/job/chaos/squad/medic/radio_help_message(mob/M)
	. = ..()
...
/datum/outfit/job/chaos/squad/medic
	name = "CHAOS Medic"
	jobtype = /datum/job/chaos/squad/medic

	id = /obj/item/card/id/dogtag/chaos


/datum/job/chaos/squad/veteran
	title = CHAOS_SQUAD_VETERAN
	paygrade = "CHAOS_S1"
	comm_title = "Vet"
	total_positions = 8
	...
	outfit = /datum/outfit/job/chaos/squad/veteran
	job_flags = JOB_FLAG_LATEJOINABLE|JOB_FLAG_ROUNDSTARTJOINABLE|JOB_FLAG_ALLOWS_PREFS_GEAR|JOB_FLAG_PROVIDES_BANK_ACCOUNT|JOB_FLAG_ADDTOMANIFEST|JOB_FLAG_PROVIDES_SQUAD_HUD|JOB_FLAG_CAN_SEE_ORDERS
	jobworth = list(/datum/job/tyranid = LARVA_POINTS_REGULAR)
	html_description = {"
		<b>Difficulty</b>: Medium<br /><br />
		<b>You answer to the</b> acting Squad Leader<br /><br />
		<b>Unlock Requirement</b>: Starting Role<br /><br />
		<b>Gamemode Availability</b>: Combat patrol and Sensor Capture<br /><br /><br />
		You are a seasoned veteran of the CHAOS. You have fought and bled for the cause, proving your self a true Son of Mars. As fitting reward for your service, you are entrusted with best arms and equipment the CHAOS can offer, and you are expected to serve as an example to your fellow soldier.
		<br /><br />
		<b>Duty</b>: Show your comrades how a true Son of Mars acts, and crush our enemies without mercy!.
	"}

/datum/job/chaos/squad/veteran/after_spawn(mob/living/carbon/new_mob, mob/user, latejoin = FALSE)
	. = ..()
	if(!ishuman(new_mob))
		return
	var/mob/living/carbon/human/new_human = new_mob
	...
	switch(playtime_mins)
		...
		if(1501 to 6000) // 25 hrs
			new_human.wear_id.paygrade = "CHAOS_S2"
		if(6001 to 18000) // 100 hrs
			new_human.wear_id.paygrade = "CHAOS_S3"
		if(18001 to 30000) // 300 hrs
			new_human.wear_id.paygrade = "CHAOS_S4"
		if(30001 to INFINITY) // 500 hrs
			new_human.wear_id.paygrade = "CHAOS_S5"

/datum/job/chaos/squad/veteran/radio_help_message(mob/M)
	. = ..()
	to_chat(M, {"\nYou are the a Veteran among the CHAOS. With your long experience (and better training and equipment), your job is to provide special weapons support to bolster the line."})

/datum/outfit/job/chaos/squad/veteran
	name = "CHAOS Veteran"
	jobtype = /datum/job/chaos/squad/veteran
	id = /obj/item/card/id/dogtag/chaos

/datum/job/chaos/squad/leader
	title = CHAOS_SQUAD_LEADER
	...
	outfit = /datum/outfit/job/chaos/squad/leader
...
/datum/job/chaos/squad/leader/after_spawn(mob/living/carbon/C, mob/user, latejoin = FALSE)
	. = ..()
	if(!ishuman(C))
		return
...
/datum/job/chaos/squad/leader/radio_help_message(mob/M)
	. = ..()
...
/datum/outfit/job/chaos/squad/leader
	name = "CHAOS Leader"
	jobtype = /datum/job/chaos/squad/leader
